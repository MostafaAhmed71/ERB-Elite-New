-- دمج خطط أسبوعية مكررة (نفس فصل + فصل دراسي + أسبوع) ثم إنشاء الفهرس الفريد
-- شغّل هذا الملف أولاً إذا فشل fix-weekly-plan-save.sql بخطأ 23505

-- ─── معاينة التكرارات (اختياري) ─────────────────────────────
SELECT education_level, grade, section, semester, week_number, COUNT(*) AS rows_count
FROM public.academic_weekly_plans
GROUP BY education_level, grade, section, semester, week_number
HAVING COUNT(*) > 1
ORDER BY rows_count DESC;

-- ─── دمج التكرارات: نبقي أحدث صف وندمج الحصص ───────────────
DO $$
DECLARE
  grp RECORD;
  v_keep_id UUID;
