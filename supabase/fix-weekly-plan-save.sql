-- إصلاح حفظ الخطة الأسبوعية (عمود semester + دالة الحفظ المشتركة)
-- شغّل الملف كاملاً في Supabase SQL Editor ثم أعد المحاولة من التطبيق
...
-- ─── دمج خطط مكررة قبل الفهرس الفريد ─────────────────────────
DO $$
DECLARE
  grp RECORD;
  v_keep_id UUID;
...
AS $$
  ...
  OR EXISTS (
    ...
    WHERE a.teacher_id = p_teacher_id
      ...
      AND a.grades_with_sections ? p_grade::text
      AND (
        jsonb_array_length(COALESCE(a.grades_with_sections -> p_grade::text, '[]'::jsonb)) = 0
        OR COALESCE(a.grades_with_sections -> p_grade::text, '[]'::jsonb) @> to_jsonb(p_section)
      )
  )
  OR EXISTS (
    SELECT 1
...
DROP POLICY IF EXISTS "academic_plans_teacher_read" ON public.academic_weekly_plans;
