...
export const PRINCIPAL_REMINDER_KINDS: {
  ...
  { id: 'platform_update', label: 'تحديث المنصة', description: 'إصلاحات وإضافات جاهزة للإرسال للمعلمين' },
...
/** إعلان واتساب فردي للمعلم */
export function buildStaffPlatformUpdateMessage(teacherName: string): string {
  return (
    `السلام عليكم ${teacherName}،\n\n` +
    `نحيطكم علماً أنه *تم إصلاح الأعطال* التي واجهها بعض الزملاء، مع تحسينات تسهّل العمل اليومي في المنصة.\n\n` +
    `*ما تم إصلاحه*\n` +
    `• حفظ *الخطة الأسبوعية* يعمل بشكل مستقر\n` +
    `• حذف الخطة الأسبوعية يحذف *الخطة كاملة* وليس الحصص فقط\n` +
    `• لا يُنشأ أكثر من *جدول دراسي لنفس الفصل* — التعديل يحدّث الجدول الموجود\n` +
    `• المواد في الملف التعليمي تُحفظ *حسب كل صف* وليس لكل الصفوف دفعة واحدة\n\n` +
    `*الإضافات الجديدة*\n` +
    `• يمكن *تعديل الملف التعليمي* بعد إنشائه من الشؤون الأكاديمية\n` +
    `• في الجدول الدراسي يظهر *نصاب كل فصل* و*النصاب الجماعي*\n` +
    `• اختيار أسبوع الخطة الأسبوعية *حرّ* وغير مربوط بأسبوع التقويم الحالي\n` +
    `• دخول المعلم: *رقم الجوال (واتساب)* أو *حساب جيميل* — دون إدخال بريد وكلمة مرور يدوياً\n` +
    `• التحديث يصل تلقائياً عند فتح التطبيق أو الموقع — *لا حاجة لحذف التطبيق وإعادة تثبيته* (آيفون وأندرويد والمتصفح)\n\n` +
    `يكفي فتح التطبيق أو تحديث الصفحة مرة بعد هذا الإعلان.\n` +
    `لأي صعوبة تواصلوا مع الدعم الفني \n\n` +
    `مع خالص التحية `
  );
}

/** نص جاهز للجروبات (واتساب) */
export const STAFF_PLATFORM_UPDATE_GROUP_MESSAGE =
  `السلام عليكم ورحمة الله وبركاته،\n\n` +
  `إخواني المعلمين،\n\n` +
  `نحيطكم علماً بأنه *تم معالجة الأعطال* التي ظهرت لدى بعض الزملاء أثناء استخدام المنصة، مع إضافة تحسينات تسهّل العمل اليومي.\n\n` +
  `*ما تم إصلاحه*\n` +
  `• حفظ الخطة الأسبوعية يعمل بشكل مستقر\n` +
  `• حذف الخطة الأسبوعية يحذف الخطة كاملة وليس الحصص فقط\n` +
  `• لا يُسمح بأكثر من جدول دراسي لنفس الفصل؛ التعديل يحدّث الجدول الموجود\n` +
  `• المواد في الملف التعليمي تُحفظ حسب كل صف على حدة\n\n` +
  `*ما أُضيف*\n` +
  `• إمكانية تعديل الملف التعليمي بعد إنشائه من الشؤون الأكاديمية\n` +
  `• عرض نصاب كل فصل والنصاب الجماعي في الجدول الدراسي\n` +
  `• اختيار أسبوع الخطة الأسبوعية بحرية، دون تقييد بأسبوع التقويم الحالي\n` +
  `• دخول المعلم برقم الجوال (واتساب) أو حساب جيميل، دون إدخال بريد وكلمة مرور يدوياً\n` +
  `• التحديث يصل تلقائياً عند فتح التطبيق أو الموقع — لا حاجة لحذف التطبيق وإعادة تثبيته على الآيفون أو الأندرويد أو من المتصفح\n\n` +
  `يكفي فتح أيقونة التطبيق أو تحديث الصفحة مرة واحدة.\n\n` +
  `لأي ملاحظة أو صعوبة، يُرجى التواصل مع الإدارة.\n\n` +
  `شاكرين تعاونكم، وتمنياتنا لكم بتوفيق مستمر.`;
...
export function buildPrincipalReminderDraft(
  ...
  switch (kind) {
    ...
    case 'platform_update':
      return buildStaffPlatformUpdateMessage(name);
    case 'custom':
