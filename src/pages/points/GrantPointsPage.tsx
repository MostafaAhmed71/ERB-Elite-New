...
import { Award, Users, User, CheckCircle2, AlertCircle, Zap, MinusCircle } from 'lucide-react';
...
export function GrantPointsPage({ embedded = false, mode = 'grant' }: GrantPointsPageProps) {
  ...
  const { data: students = [], isLoading: studentsLoading } = useQuery({
    queryKey: ['students', isTeacher ? user?.id : 'all'],
    queryFn: async () => {
    ...
    enabled: !!user && (!isTeacher || teacherAssignments !== undefined),
  });
  ...
  const availableGrades = useMemo(() => {
    if (isTeacher) {
      const scoped = filterStudentsByAssignments(students, teacherAssignments);
      return mergeGradeLists(
        teacherAssignments.map((a) => a.grade),
        scoped.map((s) => s.grade),
        catalog?.grades
      );
    }
    return catalog?.grades ?? [];
  }, [isTeacher, teacherAssignments, students, catalog?.grades]);

  const availableClasses = useMemo(() => {
    if (!filterGrade) return [];
    if (isTeacher) {
      const scoped = filterStudentsByAssignments(students, teacherAssignments);
      const fromAssign = teacherAssignments
        .filter((a) => a.grade === filterGrade)
        .map((a) => a.class_name);
      const fromStudents = scoped
        .filter((s) => s.grade === filterGrade)
        .map((s) => s.class_name);
      return mergeGradeLists(fromAssign, fromStudents, catalog?.classesByGrade[filterGrade]);
    }
    return catalog?.classesByGrade[filterGrade] ?? catalog?.allClasses ?? [];
  }, [isTeacher, teacherAssignments, students, catalog, filterGrade]);

  const scopedStudents = useMemo(() => {
    let list = isTeacher ? filterStudentsByAssignments(students, teacherAssignments) : students;
    if (filterGrade) list = list.filter((s) => s.grade === filterGrade);
    if (filterClass) list = list.filter((s) => s.class_name === filterClass);
    return list;
  }, [students, isTeacher, teacherAssignments, filterGrade, filterClass]);
  ...
  const activityWeekOn =
    ...
    selectedActivityData?.category === 'activity';

  const basePoints = customPoints !== '' ? customPoints : (selectedActivityData?.default_points ?? 0);
  const pointsToApply =
  ...
  const grantMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!selectedActivity) throw new Error('اختر نشاطاً أولاً');
    ...
    onSuccess: () => {
      ...
      if (canDirectApprove) {
        showSuccess(
          `تم ${actionLabel} ${Math.abs(signedPoints)} نقطة لـ ${selectedStudents.length} طالب بنجاح`
        );
      } else {
        showSuccess(
  ...
  const selectClass = (classStudents: DbStudent[]) => {
    ...
    <div className="space-y-6" dir="rtl">
      {!embedded && (
        <PageHeader
          ...
          subtitle={
            isTeacher
              ? `طلاب فصولك: ${formatTeacherClasses(teacherAssignments)}`
              : isDeduct
      ...
      {isTeacher && teacherAssignments.length === 0 && (
        <div className="flex items-center gap-3 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-300 text-sm">
          ...
          لم يُسند إليك أي فصل بعد. تواصل مع رائد النشاط لربطك بفصولك.
        </div>
      )}
      ...
      {!isDeduct && (
        <div className="glass-card p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-white/70 text-sm font-medium">قوالب جاهزة</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-card overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-gold-400/50 disabled:opacity-40"
                >
                  <option value="" className="bg-navy-900">
                    اختر الفصل
                  </option>
          ...
          <div className="overflow-y-auto max-h-[480px] p-3 space-y-3">
            {!filterGrade || !filterClass ? (
              <EmptyState
                illustration="students"
                title="اختر الصف والفصل"
                description="حدد الصف والفصل لعرض قائمة الطلاب"
              />
            ) : studentsLoading ? (
              ...
              <EmptyState
                ...
                description={
                  isTeacher
                    ? 'لا يوجد طلاب في هذا الفصل ضمن فصولك المسندة'
                ...
                <div key={className}>
                  <div className="flex items-center justify-between px-3 py-2 mb-1">
                    ...
                    {bulkMode && (
                      <button
                        type="button"
                        onClick={() => selectClass(classStudents)}
                        className={clsx(
                          'text-xs transition-colors',
                          isDeduct ? 'text-red-400/70 hover:text-red-400' : 'text-gold-400/70 hover:text-gold-400'
                        )}
                      >
                  ...
                  <div className="space-y-1">
                    {classStudents.map((student) => (
                      <button
                        ...
                        className={clsx(
                          'w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-right border',
        ...
        <div className="glass-card p-5 space-y-4 h-fit">
          ...
          <div className="space-y-1.5">
            <label className="text-white/60 text-xs">
              {isDeduct ? 'نقاط الخصم' : 'النقاط (اتركه فارغاً للافتراضي)'}
            </label>
            <input
              type="number"
              min={1}
